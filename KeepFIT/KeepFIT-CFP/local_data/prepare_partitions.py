"""
This script contains the code used to obtain the assembly data dataframes with text and categorical labels, partitioned
for train/evaluate.

Also, we prepare here the dataframes for prompts (zero-shot) and transferability experiments.
...
def adequate_02_messidor():
    ...
    data = []
    for iFile in range(dataframe.shape[0]):
        image_path = path_dataset + "IMAGES/" + dataframe["image_id"][iFile].replace(".jpg", ".JPG")
        categories, atributes = [], []
    ...
    print(f'02_Messidor-2 has {len(data)} images')
...
def adequate_03_idrid_segmentation():
    ...
    for iPartition in range(len(partitions)):
        for iCategory in range(len(categories_paths)):
            data = []
            files = os.listdir(
                PATH_DATASETS + path_dataset + subpath + subpath_masks + partitions[iPartition] + categories_paths[
                    iCategory])

            for iFile in files:
                image_path = path_dataset + subpath + subpath_images + \
                             partitions[iPartition] + iFile.replace(".tif", ".jpg").replace(
                    "_" + annotations_abbreviations[iCategory], "")
                mask_path = path_dataset + subpath + subpath_masks + partitions[iPartition] + categories_paths[
                    iCategory] + iFile
                data.append({"image": image_path,
                             "mask": mask_path})
...
def adequate_30_sustech():
    ...
    data_c5 = pd.read_csv(PATH_DATASETS + path_dataset + image_subpath + "c5_DR_reclassified.csv")                # 新增的标签 类5   new label
...
def adequate_31_jichi():
    ...
    image_subpath = "dmr/"  # We only have labels for train subset
...
def adequate_37_DeepDRiD():
    ...
    for i in range(0, len(paritions)):
        ...
        dataframe = pd.read_csv(
            PATH_DATASETS + path_dataset + subpath + paritions_path[i] + paritions_path[i][:-1] + ".csv")
    ...
    dataframe = pd.read_excel(PATH_DATASETS + path_dataset + subsubpath + "Challenge1_labels.xlsx")
...
def adequate_38_SynFundus(frac):
    path_dataset = '38_SynFundus-1M/'
    dataframe = pd.read_csv(PATH_DATASETS + path_dataset + 'SynFundus-1M_gen_cond.csv')
    DR_grad = {0: "no diabetic retinopathy", 1: "mild diabetic retinopathy", 2: "moderate diabetic retinopathy",
               3: "severe diabetic retinopathy", 4: "proliferative diabetic retinopathy"}
    label_categories = {'is_amd': 'age-related macular degeneration', 'is_aon': 'anomalies of the Optic Nerve',
                        'is_crp': 'Choroidal Retinal Pathology', 'is_dm': 'Degenerative Myopia',
                        'is_dme': 'no referable diabetic macular edema', 'is_em': 'epiretinal membrane',
                        'is_gc': 'glaucoma', 'is_htr': 'hypertensive retinopathy',
                        'is_pm': 'pathologic myopia', 'is_rvo': 'branch retinal vein occlusion'}
    data = []
    for i, item in dataframe.iterrows():
        print(i, end = '\r', flush=True)
        subpath = 'Part' + str(min(19, int(i/50000))) + '/'
        categories = []
        dr_grad = item['dr_grade']
        categories.append(DR_grad[dr_grad])
        for key, value in label_categories.items():
            if item[key] == 1:
                categories.append(value)
        if categories == ['no diabetic retinopathy']:
            categories = ["normal"]
        # 数据筛选  data filter
        if categories == ['no diabetic retinopathy', 'Choroidal Retinal Pathology'] or categories == ['no diabetic retinopathy', 'Degenerative Myopia'] or categories == ['no diabetic retinopathy', 'Choroidal Retinal Pathology', 'Degenerative Myopia']:
            continue
        data.append({"image": path_dataset + subpath + str(i) + '.png',
                    "atributes": [],
                    "categories": categories})
    # 随机采样  random sampling
    df_out = pd.DataFrame(data)
    df_out = df_out.sample(frac=frac)
    print(len(df_out))
    df_out.to_csv(PATH_DATAFRAME_PRETRAIN + "38_SynFundus.csv")
...
def AMD():
    path_dataset = "AMD/"
    categories = {"AMD" : "age related macular degeneration",
                  "Non-AMD" : "normal"}
    data = []
    for categorie in categories.keys():
        for iFile in os.listdir(PATH_DATASETS + path_dataset + categorie + "/"):
            data.append({"image" : path_dataset + categorie + "/" + iFile,
                         "atributes": [],
                         "categories": [categories[categorie]]})
    df_out = pd.DataFrame(data)
    df_out.to_csv(PATH_DATAFRAME_TRANSFERABILITY_CLASSIFICATION + "AMD.csv")
    print(len(data))
...
def TAOP():
    path_dataset = "TAOP/"
    data = []
    dataframe = pd.read_csv(PATH_DATASETS + path_dataset +  "label_train.csv")
    for iFile in range(dataframe.shape[0]):
        id = dataframe["id_patient"].values[iFile]
        categorie = dataframe["Disease"].values[iFile]
        image_path = path_dataset + "png/" + id + ".png"

        data.append({"image": image_path,
                     "atributes": [],
                     "categories": [str(categorie)+'c']})
    df_out = pd.DataFrame(data)
    df_out.to_csv(PATH_DATAFRAME_TRANSFERABILITY_CLASSIFICATION + "TAOP_train.csv")
    print(len(data))

    data = []
    dataframe = pd.read_csv(PATH_DATASETS + path_dataset +  "label_valid.csv")
    for iFile in range(dataframe.shape[0]):
        id = dataframe["id_patient"].values[iFile]
        categorie = dataframe["Disease"].values[iFile]
        image_path = path_dataset + "png/" + id + ".png"
        ...
        data.append({"image": image_path,
                     ...
                     "categories": [str(categorie)+'c']})
    df_out = pd.DataFrame(data)
    df_out.to_csv(PATH_DATAFRAME_TRANSFERABILITY_CLASSIFICATION + "TAOP_test.csv")
    print(len(data))
...
def adequate_mmac_segmentation():
    ...
    for iCategory in range(len(categories_paths)):
        data = []
        files = os.listdir(
            PATH_DATASETS + path_dataset + subpath + categories_paths[iCategory] + subpath_images + subpath_partitions)

        for iFile in files:
            image_path = path_dataset + subpath + categories_paths[
                iCategory] + subpath_images + subpath_partitions + iFile
            mask_path = image_path.replace(subpath_images, subpath_masks)
            data.append({"image": image_path,
                         "mask": mask_path})

        df_out = pd.DataFrame(data)
        df_out.to_csv(PATH_DATAFRAME_TRANSFERABILITY_SEGMENTATION + "mmac_" + annotations_abbreviations[
            iCategory] + "_segmentation.csv")
